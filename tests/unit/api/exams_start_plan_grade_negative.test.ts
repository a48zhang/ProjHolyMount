            ...
            bind: (..._args: any[]) => ({
                first: async <T>() => {
                    ...
                    if (sql.includes('FROM exams WHERE id = ?')) {
                        return { status: 'published', start_at: null, end_at: null, duration_minutes: null, required_plan: 'premium', required_grade_level: null, is_public: 0 } as unknown as T;
                    }
        ...
        prepare: (sql: string) => ({
            bind: (..._args: any[]) => ({
                first: async <T>() => {
                    if (sql.includes('FROM exam_assignments')) return { id: 1 } as unknown as T;
                    if (sql.includes('FROM exams WHERE id = ?')) {
                        return { status: 'published', start_at: null, end_at: null, duration_minutes: null, required_plan: null, required_grade_level: 'G7', is_public: 0 } as unknown as T;
                    }
                    return null as unknown as T;
                }
            })

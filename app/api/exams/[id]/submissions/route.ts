...
export async function GET(request: Request) {
    try {
        ...
        const rows = await ctx.env.DB
            .prepare('SELECT id, user_id, status, score_auto, score_manual, score_total, submitted_at FROM submissions WHERE exam_id = ? ORDER BY id DESC')
            .bind(examId)
            .all<any>();

        return NextResponse.json({ success: true, data: rows.results || [] });
    } catch (error) {
        console.error('获取试卷提交列表错误:', error);
